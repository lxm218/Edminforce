  ...
  render(){
    ...
    return <div {... _.omit(this.props,["list"])} style={styles.break}>
      <div style={styles.canvas}>
        <nav style={styles.canvasInner}>
        {
        this.props.list.map(function(item,n){

          let isCur = n==curState || item.href==curPath
          let uiProps = {
            uiClass: isCur && item.uiClassCur ? item.uiClassCur : item.uiClass,
            uiColor: isCur && item.uiColorCur ? item.uiColorCur : item.uiColor,
            uiSize: item.uiSize || 15,
            theme: "inline-block",
            itemStyle: { margin: "0 0 3px" }
          }

          return <RC.URL {... item} style={styles.item} onClick={self.clickHandler.bind(null, n, item.onClick)} key={n}>
