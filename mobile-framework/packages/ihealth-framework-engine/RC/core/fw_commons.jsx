...
RC.uiIcon = React.createClass({
  ...
  render() {
    ...
    return this.props.uiBgColor ? <span style={styles.canvas}>{ui}</span> : ui
  },
  // @@@@
  // @@@@
  // Styles
  ...
  baseStyles(np,ns) {
    ...
    // Start Styles
    return {
      canvas: h.assignClone( RC.cssMixins.avatar, {
  ...
  themeStyles(np,ns) {
    ...
    return {
      ...
      absCenter: {
        canvas: {
          top: "50%", left: "50%", margin: (styles.canvas.height/-2)+"px 0 0 "+(styles.canvas.height/-2)+"px"
        },
        item: {
          top: "50%", left: "50%",
