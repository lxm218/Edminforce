...
RC.Checkbox = React.createClass({
  ...
  getInitialState(){
    return {
      checked: this.props.checked || false, // Must be Boolean
      value: false
    }
  },
  reset(){
  ...
  getValue(){
    return trim((this.state.value!==false ? this.state.value : this.props.value) || "")
  },
  getChecked(){
  ...
  render() {
     ...
     */
    return <div style={styles.canvas} onClick={this.clickHandler}>
      {this.props.label}
      {this.renderAuto()}
    </div>
  },
...
RC.RadioGroup = React.createClass({
  ...
  makeRadio(radio,n){
     ...
     * <div> is used instead of <label> to overcome Web/Mobile issues
     */
    return <div style={styles.canvas} key={n} onClick={this.clickHandler.bind(null,n)}>
...
RC.Range = React.createClass({
  ...
  render() {
    ...
    return <div style={styles.canvas}>
      ...
      <input
        ...
        {... _.omit(this.props, ["onChange","value","type"])}
        value={this.getValue()}
        min={this.min}
        max={this.max}
      />
  ...
  baseStyles(np,ns) {
    ...
    return {
      ...
      progress: {
        position: "absolute", top: 0, bottom: 0, left: 0, right: (100-this.getPerCent(np,ns))+"%",
      ...
      ball: {
        ...
        width: 20, height: 20, margin: "-10px 0 0 -10px",
