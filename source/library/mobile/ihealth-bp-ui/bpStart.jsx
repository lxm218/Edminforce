...
IH.RC.PrepareBP = React.createClass({
  mixins: [IH.RC.BPSearchMixin],
  // this.data.devices has device info
  getInitialState() {
    return {
      buttonHeld: false,
      help: false,
...
IH.RC.BP5 = React.createClass({
  mixins: [IH.RC.BPComponentMixin],
  // this.data.deviceJson has BP measuring info
  render() {

    let device = this.props.device
    let deviceKnown = !!device
    if(!deviceKnown) {
      _.map(_.range(10), () => console.error('no BP device!' + device))
    }
    ...
    return <div className={"abs-full background device-back BP"+(this.props.addHeaderSpace ? " nav-margin" : "")} style={{overflow: 'hidden'}}>
