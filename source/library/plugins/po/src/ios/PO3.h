...
/* 实时测量 */
-(void)commandStartPO3WithAppSecret:(NSString*)AppSecret MeasureData:(StartPO3MeasureData)startPO3MeasureData Measure:(DisposePO3MeasureData)disposePO3MeasureData  FinishPO3MeasureData:(FinishPO3MeasureData)finishPO3MeasureData  DisposeErrorBlock:(DisposePO3ErrorBlock)disposeErrorBlock;

/* 历史数据 */
-(void)commandDisposePO3WithAppSecret:(NSString*)AppSecret DataCount:(DisposePO3DataCount)disposePO3DataCount TransferMemorryData:(StartPO3Transmission)startTransmission Memory:(DisposePO3HistoryData)disposePO3HistoryData DisposePO3WaveHistoryData:(DisposePO3WaveHistoryData)disposePO3WaveHistoryData   DisposeProgress:(DisposePO3ProgressData)progress  FinishTransmission:(FinishPO3Transmission)finishTransmission DisposeErrorBlock:(DisposePO3ErrorBlock)disposeErrorBlock;

/* 恢复出厂设置 */
-(void)commandResetPO3WithAppSecret:(NSString*)AppSecret DeviceDisposeResultBlock:(DisposePO3Block)disposeBlock DisposeErrorBlock:(DisposePO3ErrorBlock)disposeErrorBlock;

/* 查询电量 */
-(void)commandQueryBatteryInfoWithAppSecret:(NSString*)AppSecret Battery:(DisposePO3Block)disposeBlock DisposeErrorBlock:(DisposePO3ErrorBlock)disposeErrorBlock DisposeBattery:(DisposePO3Battery)disposeBattery;

/* 断开连接 */
-(void)commandEndPO3WithAppSecret:(NSString*)AppSecret CurrentConnect:(DisposePO3Block)disposeBlock DisposeErrorBlock:(DisposePO3ErrorBlock)disposeErrorBlock;
