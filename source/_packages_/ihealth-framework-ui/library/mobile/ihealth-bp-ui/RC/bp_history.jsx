...
IH.Device.BPHistory = class extends RC.MeteorData {
  ...
  getMeteorData() {
    ...
    const isReady = IH.DeviceSubs.ready()
  ...
  render() {
    return <div>
      ...
      <RC.Loading isReady={this.data.isReady || this.state.init} style={this.styles.list} loadingStyle={this.styles.list}>
      {
      this.data.m.map( (m,n) => {
  ...
  getStyles() {
    return {
      result: { top: RC.Theme.size.headerNavHeight() },
      list: Object.assign({}, RC.cssMixins.absFull, {
        top: RC.Theme.size.headerNavHeight(),
...
IH.Device.BPListItem = class extends React.Component {
  ...
  render() {
    ...
    return <IH.Device.ListItem {... this.props} header={dateHead} zoneColor={IH.Device.Color[options[zone]]} theme="BP">
      ...
      <IH.Device.ListArea time={date.format("h:mm A")}>
