...
IH.RC.BG5Mixin = {
  ...
  getInitialState() {
    return {
      pluginOutput: '',
      // deviceList: {"8CDE52425C58": {HardwareVersion: "1.0.0", ProtocolString: "com.jiuan.BPV20", name: "BP5", ModelNumber: "BP5 11070", msg: "discovery doing",
      //     name: "BP5", status: "connect"}},
      // deviceSelected: "8CDE52425C58", //null, //
      deviceList: {}, //
      deviceSelected: null, //
  ...
  startDiscovery() {
    ...
    h.BG5pluginCall(
      ...
      function(m) {
        ...
        if (obj.msg === h.pluginValues[pluginLabel]['startDiscovery'].success){
          ...
          h.BG5pluginCall(
            ...
            function(m2) {
              ...
              if (obj.msg === h.pluginValues[pluginLabel]['setDisconnectCallback'].success){
                console.log('disconnected', obj, self.state.deviceSelected)
                h.playAudio('keys.mp3');
          ...
          h.BG5pluginCall(
            ...
            function(m2) {
              ...
              if (obj.msg === h.pluginValues[pluginLabel]['getBattery'].success){
                console.log('getBattery', obj)
                self.updateDeviceStatus(obj.battery, obj.battery);
              }
            }
          )
  ...
  startMeasure(address) {
    ...
    h.BG5pluginCall(
      ...
      null,
      function(m2) {
        let commandName = 'startMeasure';
        console.info(commandName, m2);
        var obj = JSON.parse(m2);
        var status;
        switch(obj.msg) {
          case h.pluginValues[pluginLabel][commandName].stripIn:
            status = 'strip';
            break;
          case h.pluginValues[pluginLabel][commandName].getBlood:
            status = 'sample';
            break;
          case h.pluginValues[pluginLabel][commandName].resultMsg:
            status = 'result';
            self.setState({ [h.pluginValues[pluginLabel][commandName].result]: obj})

            if (_.isFunction(self.props.finishCallback)) {
              //dummy meal info until improved
              let mealTypes = ['Breakfast', 'Lunch', 'Dinner', 'Snack'];

              // console.log("finishCallback: ", obj)
              var dataToDB = _.omit(obj, 'msg');
              dataToDB.mealType = mealTypes[Math.ceil(Math.random()*mealTypes.length)-1];
              dataToDB.beforeMeal = Math.random() < 0.5;
...
IH.RC.BG5Steps = React.createClass({
  ...
  componentDidUpdate(prevProps, prevState) {
    if (_.indexOf(this.stages, this.props.stage) > _.indexOf(this.stages, prevProps.stage)) {
      h.playAudio('complete.mp3');
    } else {
      h.playAudio('bamboo.mp3');
    }
  },
  render: function() {
    var self = this;
    ...
    stages[self.props.stage].indicators
    // let icons = R.map((stage) => ({src: stage.on, isActive: true}), R.keys(indicator));
    let stageNum = {
      'connect': 0,
      'strip': 1,
      'sample': 2,
    ...
    let icons = (stageNum) => [
      {src: iGluco.assetPath("img/IconBluetoothIcon3.png"), isActive: stageNum>0},
      {src: iGluco.assetPath("img/IconStripIcon3.png"), isActive: stageNum>1},
      {src: iGluco.assetPath("img/IconSampleIcon3.png"), isActive: stageNum>2}
    ];
    ...
    if(_.contains(this.stages, self.props.stage)) {
      return <div style={h.ifMobile(this.styles.deviceHelp) }>
        <iGluco.Icons icons={icons(stageNum[self.props.stage])} onClick={
          function(i) { alert('complete previous steps first');}} />
...
IH.RC.BGResult = React.createClass({
  render: function() {
    ...
    return <div className={"absFull bg-white scroll on-top-higher"+deviceType}  id="measuring-bp">
    ...
    return <div className="line-average">
      <div style={style.circleInner}>
        <div style={{textAlign: 'center'}}>
          <div style={{fontSize: '2em', marginTop: '0.4em', marginBottom: '-0.25em', marginRight: '0.1em'}}>{Math.round(this.props.BG[h.pluginValues.BgManagerCordova.startMeasure.result])}</div>
