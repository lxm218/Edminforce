Meteor.startup(function() {
  ...
  if(!IH.Coll.Campaigns.findOne()) {
    let userGroups = [{
      ...
      count: IH.Coll.EmployeesTest.find({groups: {$in: ["Pre-Hypertension", "Hypertension"]}}).count()
    },{
      _id: 2222,
      groups: ["Obese"],
      count: IH.Coll.EmployeesTest.find({groups: {$in: ["Obese"]}}).count()
    },{
      _id: 3333,
      groups: ["Diabetes"],
      count: IH.Coll.EmployeesTest.find({groups: {$in: ["Diabetes"]}}).count()
    }]
