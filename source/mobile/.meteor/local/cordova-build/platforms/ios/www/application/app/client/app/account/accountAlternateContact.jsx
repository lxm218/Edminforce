...
Cal.ChangeAlternateContact = React.createClass({                       // 3
	...
	updateAlternateContact: function (e) {                                // 28
		e.preventDefault();                                                  // 29
		var self = this;                                                     // 30
		var form = this.refs.AlternateContactForm.getFormData();             // 31
                                                                       //
		if (form.name) {                                                     // 33
                                                                       //
			// Reset Account Password using token                               //
			this.setState({ waiting: true });                                   // 36
			var alternateContact = {                                            // 37
				name: form.name,                                                   // 38
				phone: form.phone,                                                 // 39
			...
			Meteor.call('SetAlternateContact', Meteor.userId(), alternateContact, function (err, result) {
				...
				if (!!err) {                                                       // 48
					...
					Dispatcher.dispatch({                                             // 57
						actionType: "ALTERNATE_CONTACT_CHANGE_SUCCESS"                   // 58
					});                                                               //
					return;                                                           // 60
				}                                                                  //
			});                                                                 //
	...
	checkButtonState: function (e) {                                      // 88
		var form = this.refs.AlternateContactForm.getFormData();             // 89
