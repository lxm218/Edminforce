...
App.Neutral.LeftHome = React.createClass({
  ...
  click() {
    alert("Sorry, the webapp framework is currently not available.")
    // FlowRouter.go("home-webapp")
  },
  hoverStart() {
    this.setState({
      isHover: true
  ...
  render() {
    ...
    return <div style={styles.area}>
      ...
      <figure style={styles.hero} onClick={this.click} onMouseEnter={this.hoverStart} onMouseLeave={this.hoverEnd}>
        <p style={styles.text}>
  ...
  baseStyles(np, ns) {
    ...
    return {
      area: h.assignClone( [area, App.CSS.brand1Gradient()],{
        left: 0, right: "50%",
      }),
      back: h.assignClone( [back,RC.cssMixins.absFull], {
        opacity: ns.isHover ? 1 : 0,
        background: "rgba(0,0,0,.1)",
      }),
      hero: h.assignClone( hero, {
...
App.Neutral.RightHome = React.createClass({
  ...
  baseStyles(np, ns) {
    let white = RC.Theme.color.white
    return {
      area: h.assignClone( [area, App.CSS.brand2Gradient()],{
        left: "50%", right: 0,
      }),
      back: h.assignClone( [back, RC.cssMixins.absFull], {
        opacity: ns.isHover ? 1 : 0,
