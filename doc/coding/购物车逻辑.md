购物车逻辑
=========


###Reference

```
参考:
https://docs.mongodb.org/manual/tutorial/perform-two-phase-commits/
http://docs.mongodb.org/ecosystem/use-cases/inventory-management/

http://learnmongodbthehardway.com/schema/chapter9/
http://learnmongodbthehardway.com/schema/chapter10/


用到购物车和事务逻辑的模块：

1.register class (normal)
2.register class (book the same time)
3.cancel class
4.change class


```
###购物车的类型

```
由于涉及到 恢复执行和回滚逻辑 为减少复杂性方便判断  购物车分三种类型

register
cancel
change

每种类型的购物车状态有所区别
1.register


2.cancel



3.change
  a.欠费
  b.退费



```

###购物车的状态


```
  
    一个购物车从添加到完成或取消作为一个事务
    
    购物车的状态：
    
    [normal flow]
    
    1.active  初始创建的状态 用户永远只有一个active的购物车， 数据处于一致状态
    
    2 pending 当用户开始添加item到购物车时 并修改class可用数量时设置此状态
    
    3.1 checking 开始pay
            此后超时分两种  pay-in-store 24h   pay-now 15m？
            
    3.2 applied 在这个状态 已支付完毕 购物车和class课程的数目已经一致， 但后面还有一些额外的事要做
            
            1.删除 class表中 对应的carted信息
            2.为classRegister表添加课程注册信息，
           
            3.修改 class的student表 更新学生信息 方便查询
            4.更改 swimmer中的class信息 方便查询
            
    4. done   购物行为完成        
    
    
    [expired flow]（由定时任务处理）
    
    5. expiring 
    6. expired
    
    [cancel flow]用户在未支付前主动取消单个后全部课程（pay-in-store情况，一天后才会自动过期）
    
    7. canceling
    8. canceled
                    

```

###故障处理 Failure Scenarios

#####恢复执行 Recovery Operations
```
在以下状态时 尚未执行或执行了部分而中断：

pending  定时任务检查 继续后面的步骤   确保反复执行结果唯一 
applied  定时任务检查 继续后面的步骤   确保反复执行结果唯一

```
#####回滚 Rollback Operations

```
checking状态
cancel   未支付成功或主动取消  执行逆向过程恢复class可用数目
expired  执行逆向过程恢复class可用数目

```
####定时任务

```
  1. 超时的active
     超时的pending 回滚， 未超时则在程序中继续往下执行
     超时的applied=>done
  2. 检查class中对应的购物车是否已经不存在  
     按照删除顺序应该不会出现但http://docs.mongodb.org/ecosystem/use-cases/inventory-management/中有这个逻辑
  
 
```
 
 

### mongodb 数据结构 
```
仅列出关键字段

暂不使用classRegister表 以后为了查询方便可考虑使用

1.classes存储课程信息
classes:{
  seatsTotal 总数
  seatsRemain:剩余名数 初值同seatsTotal  //
  carted:[
    {
      cartId,
      type  //标记购物车类型 方便查询
      swimmerId,
      quantity,  //定义quantity是为了使用负数表示取消class或change class时使用
      timestamp  
    }
  ],
  //students:[] // 学生列表  由于涉及事务处理 会增加复杂性此字段删除
}  


2.购物车信息
shoppingCarts:{
  accountId
  sessionId
  type:'register'
  items:[
    { 
      swimmerId,classId, quantity,      //key
      swimmer, class1, class2, class3  //这些为详细信息 方便查询
    }
  ]
}

//3.swimmer信息 由于涉及事务处理 会增加复杂性此字段删除
//swimmers:{
//  classes:[] //id列表  仅为方便查询 在applied阶段更新
//}

4.classRegister 信息
{
  sessionId 
  swimmerId
  classId
  carted:['cartId']
  status://  normal/changing/cancel
}



```
###操作流程以及购物车的数据格式

```
1.注册课程 

先占用 若未支付再恢复数量
cart={
  
  items:[
    {
      type:'register' //尽管已有quantity 加这个属性用于方便区分其他类型
      classId
      swimmerId
      quantity:1
      
      swimmer
      class1
      class2
      class3
    }
  ]
}

正常流程：
a. 创建购物车 或 获取active的购物车
b.classes表 class数量减去1
c.classesRegister 插入一个register项
d.支付
e.清空classes表 和classesRegister 中的事务纪录 carted

b和c需要同时完成 或同时不完成
异常

购物车的修改


  
2. 取消课程：  注册数量标记为－1

用户点击确认
step1: 创建购物车直接为pending状态 ＝》直接恢复class数量
若成功则＝》置为 applied
若失败 ＝》 有定时程序在pending状态恢复执行 
仅当退费成功 再置为done  
退费可能不是即时的  

定时任务逻辑：
无超时逻辑
cart处于pending状态且为cancel类型  恢复class数量 置为applied



cart={
  
  items:[
    {
      type:'cancel' 
      classId
      swimmerId
      quantity:-1
      
      swimmer
      class
    }
  ]
}

3. change课程：一个－1 一个+1 一个事务过程 类似银行转账过程
根据用户是否欠费做不同处理

对于用户需补交费用
a.先占用要改变到的class toClass的数量
b.再支付 
c.支付成功则 释放fromClass的数量 
d.支付失败之后超时   释放占用的toClass的数量

对于需退用户费用
占用toClass ＝》释放fromclass ＝》退款

cart={
  
  items:[
    {
      type:'change'
      
      swimmerId
      fromClassId
      toClassId
      
      swimmer
      fromClass
      toClass
      
    }
  ]
}


```

