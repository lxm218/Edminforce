...
RC.HeaderNav = class extends RC.CSS {
  ...
  _toggleFull() {
    ...
    if (_.isFunction(this.props.fullNavToggleFunc))
      this.props.fullNavToggleFunc(!this.state.obj.get("fullNav"))
  }
  goBack(home) {
    if (home) {
      ...
      if (FlowRouter.current().path==FlowRouter.LastRoute[FlowRouter.LastRoute.length-1])
        FlowRouter.LastRoute.pop()
      FlowRouter.BackButton = true
      FlowRouter.go( FlowRouter.LastRoute[FlowRouter.LastRoute.length-1] || "/" ) // APH Mobile App (Unique) Bug Catch
    }
  }
  // @@
  // @@
  ...
  baseStyles(np,ns) {
    ...
    return {
      area: {
        ...
        borderTop: RC.showStatusBar() ? `solid 20px rgba(0,0,0,.1)` : "none",
        height: RC.headerNavHeight(),
      ...
      xContain: {
        ...
        top: RC.showStatusBar() ? 20 : 0, right: 0,
        width: 50, height: RC.headerNavHeight(true),
        transition: "all .4s ease",
        ":on": {
          top: (RC.showStatusBar() ? 20 : 0)+10, right: 5
        }
      },
...
RC.HeaderNav.propTypes = Object.assign({}, RC.HeaderNav.propTypes, {
  ...
  fullNavToggleFunc: React.PropTypes.func
})
