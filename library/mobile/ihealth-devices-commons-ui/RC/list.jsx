  ...
  render() {
    ...
    if (s) {
      switch (deviceType) {
        ...
        break
        case "AM":
          const lastVal = DbTools.renameKeys(DbTools.keyMap.am, s.lastValue)
          const goal = s.goal || IH.Device.Defaults.AM.goal
          const pc = (isNaN(lastVal.step) ? 0 : lastVal.step)/goal

          score = h.amGetZone(pc)
          // scoreName = `Reached ${Math.round(pc*100)}%`
          measurement = [
            <span style={this.styles.big} key={0}>{h.numberFormat(lastVal.step)}</span>,
            <span style={this.styles.small} key={1}>steps</span>,
            <span style={this.styles.big} key={2}>{h.amCalculateCalories(profile, lastVal.calorie, lastVal.time)}</span>,
            <span style={this.styles.small} key={3}>kcal</span>,
            <span style={this.styles.big} key={4}>{h.amCalculateDistance(lastVal.stepsize, lastVal.step, profile.metricsUnit)}</span>,
    ...
    return <IH.Device.ListItem {... props} zoneColor="#FFF" onClick={this._goList.bind(this)} theme="summary">
      <span style={h.assignPseudos(this.styles.dot,`:score${score}`)} />
  ...
  getStyles() {
    ...
    return {
      ...
      icon: Object.assign({}, RC.cssMixins.absFull, {
        backgroundImage: "url(/packages/ihealth_devices-commons-ui/assets/devices-knockout.png)",
        backgroundSize: "70px 420px",
        ":BP": {
          backgroundPosition: "50% -16px", backgroundColor: IH.Device.Color.BP
        },
        ":BG": {
          backgroundPosition: "50% -366px", backgroundColor: IH.Device.Color.BG
        },
        ":AM": {
        ...
        ":PO": {
          backgroundPosition: "50% -86px", backgroundColor: IH.Device.Color.PO
        },
      }),
      score: {
      },
      dot: {
        position: "absolute", top: 5, left: 37, zIndex: 10,
...
IH.Device.ListItem = class extends RC.CSS {
  ...
  render() {
    ...
    return <div {... this.props} style={styles.area}>
      ...
      {this.props.header ? <div style={styles.header} onClick={this.preventDefault}>{this.props.header}</div> : null}
  ...
  themeStyles() {
    return {
      summary: {
        area: {
          textAlign: "right",
          minHeight: 50, padding: "10px 10px 10px 64px", margin: "0 0 0 auto",
