...
iHealthBP5.prototype = {
  ...
  connect: function(cbFail, cbSuccess) {
    ...
    var startDiscoverySuccessCB = function(res){
        // ##
        // Success Function
        bp5Log.trace("Success: "+res) // DEVELOPMENT MODE
          ...
          if (json.address && json.name && json.name.slice(0,3) === "BP5") {
            ...
            if (!h.getPlatform("ios") ) {
              if(_.isFunction(BpManagerCordova.connectDevice)) {
                BpManagerCordova.connectDevice(function (res) {
                  ...
                  bp5Log.warn("is ios connectDevice", res)
  ...
  start: function(finishCallback){
    ...
    var successcb = function(msg) {
      console.log('discoverty stopped and start measure')
      startMeasure();
    };

    BpManagerCordova.stopDiscovery( successcb, msg => console.log('fail #{msg}'), appsecret, this.macId)
  },
