...
h = {
  ...
  getDataTypeFromDeviceType({deviceType, msg}) {
    const VALID_DEVICE_TYPES = [ 'BP', 'BG', 'AM'];
    if (typeof deviceType !== 'string')
      return undefined;

    if (deviceType === 'AM') {
      // dataObj.msg must be 'activity' or 'sleep'
      if (_.isUndefined(msg) || !_.contains(['activity', 'sleep'], msg) ) {
        return undefined
      } else {
        // return 'Activity' or 'Sleep'
        return h.capitalize(msg);
      }
    } else {
      return deviceType
    }
...
if (Meteor.isClient){
  ...
  h.numberFormat = function(num, decimals){
    ...
    if (isNaN(num) && num!==null)
      return num || 0 // If string, just return, otherwise return 0
