...
IH.RC.User = React.createClass({
    ...
    switch (this.state.action) {
      case "login":
        return <RC.Form onSubmit={this.login} onKeyUp={this.checkButtonState} ref="loginForm">
          <RC.Input {... Object.assign({},inputProps, {name: "username", label: "Username or E-mail"})} />
          <RC.Input {... Object.assign({},inputProps, {name: "password", label: "Password", type: "password", style: {marginBottom: 20}})} />
  ...
  baseStyles(np,ns) {
    let navSize = 0
    if (_.isFunction(RC.headerNavHeight))
      navSize = RC.headerNavHeight()
