...
Meteor.users.allow({
  update: function (userId, doc, fields, modifier) {
    if (userId && doc._id === userId) {
      var allowed = ["devices.id","profile.","updatedAt"]
...
Meteor.publish("userData", function(userId) {
  ...
  if((userId === this.userId) || Roles.userIsInRole(this.userId, "admin", Roles.GLOBAL_GROUP)) {
    return Meteor.users.find({_id: userId},
      {
        fields: {
          profile:1, roles:1, username:1, emails: 1,
          devices: 1
        }
      })
  }
